...
Documentshapsot object allows us to check if a document exists at this query using the **.exists** property which returns a boolean.
We can the the actual properties on the object by calling **.data()** method which return us a JSON object of the document.

#### querySnapshot object

We get a querysnapshot object from our collection reference object.
We can check if there are any document in the collection by calling **.empty** returns a boolean.

We can all the document by calling **.docs** property.
